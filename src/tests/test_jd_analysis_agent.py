"""
Test script for JD Analysis Agent.
...
def create_test_request() -> WorkflowRequest:
    ...
    return WorkflowRequest(
        ...
        # scoring_weights_s3_url="h2-2025-backend-lead-2235/scoring-weights.json",
        weights={
            "jd_score_weight": 4.0,    
...
def main():
    ...
    try:
        ...
        # Create JD Analysis Agent
        agent = JDAnalysisAgent(llm)
        
        # Run analysis
        logger.info("Starting JD Analysis...")
        updated_state = agent.analyze_resume(state)
        
        # Print results
        logger.info("\n=== JD Analysis Results ===")
        logger.info(json.dumps(updated_state, indent=2))
        ...
        if updated_state.get('jd_analysis_url'):
            s3_client = S3Client()
            analysis_data = s3_client.get_object(os.getenv('S3_BUCKET_NAME'), updated_state['jd_analysis_url'])
            if analysis_data:
                logger.info("\n=== Detailed Analysis ===")
